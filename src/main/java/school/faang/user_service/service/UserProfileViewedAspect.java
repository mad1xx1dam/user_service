...
//&& hasContactReference(user)
        if (viewerId != 0 && notSameUser(viewerId, profileOwnerId)) {
            log.info("User profile viewed : viewerId: {}, profileOwnerId: {}", viewerId,
                    profileOwnerId);
            publisher.sendMessage(viewerId, profileOwnerId);
        }
    }
    ...
    private boolean hasContactReference(User user) {
